...
def background(colorlist, limite = 50):
    cdef int sum1 = 0, sum2 = 0, limit = limite
...
def color_function(long[:,:] matrix, colors = 256):
    cdef int number_colors = colors
    color = [0]*(number_colors)
    cdef int ni = matrix.shape[0], nj = matrix.shape[1]
    for i in range(ni):
        for j in range(nj):
            color[matrix[i,j]] += 1
    return color
...
def my_imbin(long[:,:]image, limite):
    cdef int limit = limite
    cdef int ni = image.shape[0], nj = image.shape[1]
    for i in range(ni):
        for j in range(nj):
            if(image[i,j] > limit):
                image[i,j] = 255
...
def my_label(long[:,:] image, framework):
    ...
    cdef int label = 1
    cdef int scene, ni = image.shape[0], nj = image.shape[1]
    if(framework == 'white'):
     scene = 0
    else:
     scene = 255
